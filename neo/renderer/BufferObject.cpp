...
static constexpr GLenum bufferUsage = GL_DYNAMIC_DRAW_ARB;
...
bool idVertexBuffer::AllocBufferObject( const void * data, const int allocSize ) {
	...
	qglBufferDataARB( GL_ARRAY_BUFFER_ARB, numBytes, nullptr, bufferUsage );
...
void idVertexBuffer::Update( const void * data, const int updateSize ) const {
	...
	GLuint bufferObject = reinterpret_cast< GLuint >( apiObject );
	qglBindBufferARB( GL_ARRAY_BUFFER_ARB, bufferObject );
	qglBufferSubDataARB( GL_ARRAY_BUFFER_ARB, GetOffset(), static_cast<GLsizeiptrARB>(numBytes), data );
/*
...
void * idVertexBuffer::MapBuffer(const bufferMapType_t mapType ) const {
	...
	if ( mapType == BM_READ ) {
		...
		if ( buffer != nullptr) {
			buffer = static_cast<byte*>(buffer) + GetOffset();
		}
...
bool idIndexBuffer::AllocBufferObject( const void * data, const int allocSize ) {
	...
	// these are rewritten every frame
	qglBufferDataARB( GL_ELEMENT_ARRAY_BUFFER_ARB, numBytes, nullptr, bufferUsage );
	apiObject = reinterpret_cast< void * >( bufferObject );
...
void idIndexBuffer::Update( const void * data, const int updateSize ) const {
	...
	GLuint bufferObject = reinterpret_cast< GLuint >( apiObject );
	qglBindBufferARB( GL_ELEMENT_ARRAY_BUFFER_ARB, bufferObject );
	qglBufferSubDataARB( GL_ELEMENT_ARRAY_BUFFER_ARB, GetOffset(), static_cast<GLsizeiptrARB>(numBytes), data );
/*
	void * buffer = MapBuffer( BM_WRITE );
	CopyBuffer( (byte *)buffer + GetOffset(), (byte *)data, numBytes );
	UnmapBuffer();
...
bool idJointBuffer::AllocBufferObject( const float * joints, const size_t numAllocJoints ) {
	...
	qglBufferDataARB( GL_UNIFORM_BUFFER, numBytes, nullptr, GL_STREAM_DRAW_ARB );
