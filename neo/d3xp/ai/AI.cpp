...
*/
	...
	if(ai_showHealth.GetBool()) {
		...
		gameRenderWorld->DrawText( va( "%d", static_cast<int>(health)), this->GetEyePosition()+aboveHead, 0.5f, colorWhite, gameLocal.GetLocalPlayer()->viewAngles.ToMat3() );
	}
}
...
float idAI::TravelDistance( const idVec3 &start, const idVec3 &end ) const {
	...
	if ( !aas ) {
		...
		if ( ai_debugMove.GetBool() ) {
			...
			gameRenderWorld->DrawText( va( "%d", static_cast<int>(dist) ), ( start + end ) * 0.5f, 0.1f, colorWhite, gameLocal.GetLocalPlayer()->viewAngles.ToMat3() );
		}
...
bool idAI::NewWanderDir( const idVec3 &dest ) {
	...
	olddir = idMath::AngleNormalize360( static_cast<int>(current_yaw / 45) * 45 );
