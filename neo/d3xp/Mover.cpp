...
================
*/
void idMover::Event_PostRestore(const int start, const int total, const int accel, const int decel, const int useSplineAng ) {
...
================
*/
void idMover::VectorForDir(const float angle, idVec3 &vec ) {
	idAngles ang;
...
==============================
*/
void idMover::ClientThink( const int curTime, const double fraction, const bool predict ) {
...
void idMover::SetGuiState( const char *key, const char *val ) const {
	...
	for ( size_t i = 0; i < guiTargets.Num(); i++ ) {
...
================
*/
void idMover::UpdateMoveSound(const moveStage_t stage ) {
	switch( stage ) {
...
void idMover::BeginMove( idThread *thread ) {
	...
	move_delta = move_delta * ( 1000.0f / ( static_cast<float>(move_time) - ( at + dt ) * 0.5f ) );
...
================
*/
void idMover::UpdateRotationSound(const moveStage_t stage ) {
	switch( stage ) {
		case ACCELERATION_STAGE: {
			StartSound( "snd_accel", SND_CHANNEL_BODY2, 0, false, nullptr);
			StartSound( "snd_move", SND_CHANNEL_BODY, 0, false, nullptr);
	...
	switch( rot.stage ) {
		...
		case LINEAR_STAGE: {
			if ( !stopRotation && !rot.deceleration ) {
				physicsObj.SetAngularExtrapolation( static_cast<extrapolation_t>(EXTRAPOLATION_LINEAR | EXTRAPOLATION_NOSTOP), gameLocal.slow.time, rot.movetime, ang, rot.rot, ang_zero );
			} else {
		...
		case FINISHED_STAGE: {
			...
			if ( stopRotation ) {
				...
				physicsObj.SetAngularExtrapolation( static_cast<extrapolation_t>(EXTRAPOLATION_LINEAR | EXTRAPOLATION_NOSTOP), gameLocal.slow.time, 0, ang, rot.rot, ang_zero );
			}
...
================
*/
void idMover::BeginRotation( idThread *thread, const bool stopwhendone ) {
	moveStage_t stage;
	idAngles	ang;
	int			at;
	int			dt;
	...
	angle_delta = angle_delta * ( 1000.0f / ( static_cast<float>(move_time) - ( at + dt ) * 0.5f ) );
...
================
*/
void idMover::Event_SetMoveSpeed(const float speed ) {
	if ( speed <= 0 ) {
...
================
*/
void idMover::Event_SetMoveTime(const float time ) {
	if ( time <= 0 ) {
...
================
*/
void idMover::Event_SetAccellerationTime(const float time ) {
	if ( time < 0 ) {
...
================
*/
void idMover::Event_SetDecelerationTime(const float time ) {
	if ( time < 0 ) {
...
================
*/
void idMover::Event_MoveDir(const float angle, const float distance ) {
	idVec3 dir;
	idVec3 org;
...
================
*/
void idMover::Event_MoveAccelerateTo(const float speed, const float time ) {
	float v;
	idVec3 org, dir;
	int at;
...
================
*/
void idMover::Event_MoveDecelerateTo(const float speed, const float time ) {
	float v;
	idVec3 org, dir;
	int dt;
...
================
*/
void idMover::Event_RotateDownTo(const int axis, const float angle ) {
	idAngles ang;
...
================
*/
void idMover::Event_RotateUpTo(const int axis, const float angle ) {
	idAngles ang;
...
================
*/
void idMover::Event_Bob(const float speed, const float phase, idVec3 &depth ) {
	idVec3 org;

	physicsObj.GetLocalOrigin( org );
	physicsObj.SetLinearExtrapolation( static_cast<extrapolation_t>(EXTRAPOLATION_DECELSINE | EXTRAPOLATION_NOSTOP), speed * 1000 * phase, speed * 500, org, depth * 2.0f, vec3_origin );
}
...
================
*/
void idMover::Event_Sway(const float speed, const float phase, idAngles &depth ) {
	...
	physicsObj.SetAngularExtrapolation( static_cast<extrapolation_t>(EXTRAPOLATION_DECELSINE | EXTRAPOLATION_NOSTOP), duration * 1000.0f * phase, duration * 1000.0f, ang, angSpeed, ang_zero );
}
...
void idMover::ReadFromSnapshot( const idBitMsg &msg ) {
	...
	move.stage = static_cast<moveStage_t>(msg.ReadBits(3));
	rot.stage = static_cast<moveStage_t>(msg.ReadBits(3));
...
================
*/
void idMover::SetPortalState(const bool open ) {
	assert( areaPortal );
...
void idElevator::Think() {
	...
	if ( state == INIT ) {
		...
		for ( size_t i = 0; i < floorInfo.Num(); i++ ) {
...
================
*/
void idElevator::OpenFloorDoor(const int floor ) {
...
================
*/
floorInfo_s *idElevator::GetFloorInfo(const int floor ) {
	for ( size_t i = 0; i < floorInfo.Num(); i++ ) {
...
================
*/
void idElevator::Event_GotoFloor(const int floor ) {
	floorInfo_s *fi = GetFloorInfo( floor );
	if ( fi ) {
		DisableAllDoors();
		CloseAllDoors();
...
===============
*/
void idMover_Binary::GetMovedir(const float angle, idVec3 &movedir ) {
	if ( angle == -1 ) {
		movedir.Set( 0, 0, 1 );
	} else if ( angle == -2 ) {
		movedir.Set( 0, 0, -1 );
...
===============
*/
void idMover_Binary::UpdateMoverSound(const moverState_t state ) {
	if ( moveMaster == this ) {
		switch( state ) {
			case MOVER_POS1:
				break;
...
===============
*/
void idMover_Binary::SetMoverState(const moverState_t newstate, const ID_TIME_T time ) {
	idVec3 	delta;
...
================
*/
void idMover_Binary::MatchActivateTeam(const moverState_t newstate, const ID_TIME_T time ) {
	idMover_Binary *slave;
...
================
*/
void idMover_Binary::Enable(const bool b ) {
	enabled = b;
}
...
================
*/
void idMover_Binary::Event_MatchActivateTeam(const moverState_t newstate, const ID_TIME_T time ) {
...
================
*/
void idMover_Binary::UpdateBuddies(const int val ) {
	int i, c;
...
================
*/
void idMover_Binary::InitSpeed( idVec3 &mpos1, idVec3 &mpos2, const float mspeed, const float maccelTime, const float mdecelTime ) {
	idVec3		move;
	float		distance;
	float		speed;
...
================
*/
void idMover_Binary::InitTime( idVec3 &mpos1, idVec3 &mpos2, const float mtime, const float maccelTime, const float mdecelTime ) {
...
================
*/
void idMover_Binary::SetBlocked(const bool b ) {
...
void idMover_Binary::ReadFromSnapshot( const idBitMsg &msg ) {
	...
	physicsObj.ReadFromSnapshot( msg );
	moverState = static_cast<moverState_t>(msg.ReadBits(3));
	ReadBindFromSnapshot( msg );
...
================
*/
void idMover_Binary::SetPortalState(const bool open ) {
	assert( areaPortal );
	gameLocal.SetPortalState( areaPortal, open ? PS_BLOCK_NONE : PS_BLOCK_ALL );
}
...
================
*/
void idDoor::ClientThink( const int curTime, const double fraction, const bool predict ) {
	idVec3 masterOrigin;
	idMat3 masterAxis;
...
================
*/
void idDoor::SetAASAreaState(const bool closed ) {
		...
		if ( other->IsType( idDoor::Type ) ) {
			...
			if ( other == moveMaster ) {
				if ( door->sndTrigger == nullptr) {
					...
					if ( sndtemp != nullptr && *sndtemp != NULL ) {
...
======================
*/
void idDoor::CalcTriggerBounds(const float size, idBounds &bounds ) {
	idMover_Binary	*other;
	int				i;
	int				best;
...
void idDoor::Event_SpawnDoorTrigger() {
	...
	const char *sndtemp = spawnArgs.GetString( "snd_locked" );
	if ( spawnArgs.GetInt( "locked" ) && sndtemp != nullptr && *sndtemp != NULL ) {
		PostEventMS( &EV_Door_SpawnSoundTrigger, 0 );
	}
...
================
*/
void idDoor::Event_Lock(const int f ) {
	Lock( f );
}
...
================
*/
void idPlat::ClientThink( const int curTime, const double fraction, const bool predict ) {
	InterpolatePhysicsOnly( fraction );
...
void idRotater::Spawn() {
	...
	physicsObj.SetAngularExtrapolation( static_cast<extrapolation_t>(EXTRAPOLATION_LINEAR | EXTRAPOLATION_NOSTOP), gameLocal.slow.time, 0, GetPhysics()->GetAxis().ToAngles(), ang_zero, ang_zero );
	SetPhysics( &physicsObj );
...
void idRotater::Event_Activate( idEntity *activator ) {
	...
	physicsObj.SetAngularExtrapolation( static_cast<extrapolation_t>(EXTRAPOLATION_LINEAR | EXTRAPOLATION_NOSTOP), gameLocal.slow.time, 0, physicsObj.GetAxis().ToAngles(), delta, ang_zero );
}
...
void idBobber::Spawn() {
	...
	physicsObj.SetLinearExtrapolation( static_cast<extrapolation_t>(EXTRAPOLATION_DECELSINE | EXTRAPOLATION_NOSTOP), phase * 1000, speed * 500, GetPhysics()->GetOrigin(), delta * 2.0f, vec3_origin );
	SetPhysics( &physicsObj );
}
...
void idPendulum::Spawn() {
	...
	physicsObj.SetLinearExtrapolation( EXTRAPOLATION_NONE, 0, 0, GetPhysics()->GetOrigin(), vec3_origin, vec3_origin );
	physicsObj.SetAngularExtrapolation( static_cast<extrapolation_t>(EXTRAPOLATION_DECELSINE | EXTRAPOLATION_NOSTOP), phase * 1000, 500/freq, GetPhysics()->GetAxis().ToAngles(), idAngles( 0, 0, speed * 2.0f ), ang_zero );
	SetPhysics( &physicsObj );
}
