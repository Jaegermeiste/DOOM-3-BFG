...
ID_INLINE idPolynomial::idPolynomial(const int d ) {
	degree = -1;
	allocated = 0;
	coefficient = nullptr;
	Resize( d, false );
...
ID_INLINE idPolynomial::idPolynomial(const float a, const float b ) {
	degree = -1;
	allocated = 0;
	coefficient = nullptr;
	Resize( 1, false );
...
ID_INLINE idPolynomial::idPolynomial(const float a, const float b, const float c ) {
	degree = -1;
	allocated = 0;
	coefficient = nullptr;
	Resize( 2, false );
...
ID_INLINE idPolynomial::idPolynomial(const float a, const float b, const float c, const float d ) {
	degree = -1;
	allocated = 0;
	coefficient = nullptr;
	Resize( 3, false );
...
ID_INLINE idPolynomial::idPolynomial(const float a, const float b, const float c, const float d, const float e ) {
	degree = -1;
	allocated = 0;
	coefficient = nullptr;
	Resize( 4, false );
...
ID_INLINE idPolynomial idPolynomial::operator+( const idPolynomial &p ) const {
	...
	if ( degree > p.degree ) {
		...
		for ( i = 0; std::cmp_less_equal(i, p.degree); i++ ) {
			n.coefficient[i] = coefficient[i] + p.coefficient[i];
		}
		for ( ; std::cmp_less_equal(i, degree); i++ ) {
		...
		for ( ; std::cmp_less_equal(i, p.degree); i++ ) {
...
ID_INLINE bool idPolynomial::Compare( const idPolynomial &p ) const {
	...
	for ( size_t i = 0; std::cmp_less_equal(i, degree); i++ ) {
		if ( std::not_equal_to<>()(coefficient[i], p.coefficient[i]) ) {
			return false;
		}
	}
	return true;
...
ID_INLINE void idPolynomial::Zero(const int d ) {
	Resize( d, false );
	for ( size_t i = 0; std::cmp_less_equal(i, degree); i++ ) {
		coefficient[i] = 0.0f;
	}
...
ID_INLINE size_t idPolynomial::GetRoots3(const float a, float b, float c, float d, float *roots ) {
	...
	const float f = (1.0f / 3.0f) * (3.0f * c - b * b);
	const float g = (1.0f / 27.0f) * (2.0f * b * b * b - 9.0f * c * b + 27.0f * d);
	const float halfg = 0.5f * g;
	const float ofs = (1.0f / 3.0f) * b;
	float ds = 0.25f * g * g + (1.0f / 27.0f) * f * f * f;

	if ( ds < 0.0f ) {
		const float dist = idMath::Sqrt((-1.0f / 3.0f) * f);
		const float angle = (1.0f / 3.0f) * idMath::ATan(idMath::Sqrt(-ds), -halfg);
		const float cs = idMath::Cos(angle);
		const float ss = idMath::Sin(angle);
...
ID_INLINE size_t idPolynomial::GetRoots4(const float a, float b, float c, float d, float e, float *roots ) {
	...
	const float ds = 0.25f * b * b - c + y;

	if ( ds < 0.0f ) {
		return 0;
	} else if ( ds > 0.0f ) {
		const float r = idMath::Sqrt(ds);
		t1 = 0.75f * b * b - r * r - 2.0f * c;
		t2 = ( 4.0f * b * c - 8.0f * d - b * b * b ) / ( 4.0f * r );
		const float tp = t1 + t2;
		const float tm = t1 - t2;
