...
static constexpr int MAX_TAGS = 256;
...
void *operator new(const size_t s, const memTag_t tag ) {
...
ID_INLINE idTempArray<T>::idTempArray( size_t num ) {
	this->num = num;
	buffer = static_cast<T*>(Mem_Alloc(num * sizeof(T), TAG_TEMP));
}
...
constexpr auto BLOCK_ALLOC_ALIGNMENT = 16;
...
public:
	...
	ID_INLINE void		SetFixedBlocks( size_t numBlocks );
	ID_INLINE void		FreeEmptyBlocks();

	ID_INLINE _type_ *	Alloc();
	ID_INLINE void		Free( _type_ *element );

	[[nodiscard]] size_t				GetTotalCount() const { return total; }
	[[nodiscard]] size_t				GetAllocCount() const { return active; }
	[[nodiscard]] size_t				GetFreeCount() const { return total - active; }
...
private:
	...
	public:
		...
		size_t			freeCount;	// number of free elements in this block (temp used only by FreeEmptyBlocks)
	};
...
ID_INLINE _type_ * idBlockAlloc<_type_,_blockSize_,memTag>::Alloc() {
	...
	_type_ * t = reinterpret_cast<_type_*>(element->buffer);
	if ( clearAllocs ) {
...
ID_INLINE void idBlockAlloc<_type_,_blockSize_,memTag>::Free( _type_ * t ) {
	...
	element_t * element = reinterpret_cast<element_t*>(t);
	element->next = free;
	free = element;
	active--;
#endif
...
*/
template<class _type_, int _blockSize_, memTag_t memTag>
ID_INLINE void idBlockAlloc<_type_,_blockSize_,memTag>::SetFixedBlocks(const size_t numBlocks ) {
	int currentNumBlocks = 0;
	for ( idBlock * block = blocks; block != nullptr; block = block->next ) {
		currentNumBlocks++;
	}
	for ( int i = currentNumBlocks; std::cmp_less(i, numBlocks); i++ ) {
		AllocNewBlock();
	}
	allowAllocs = false;
}
...
ID_INLINE void idBlockAlloc<_type_,_blockSize_,memTag>::AllocNewBlock() {
	idBlock * block = static_cast<idBlock*>(Mem_Alloc(sizeof(idBlock), memTag));
	block->next = blocks;
	blocks = block;
	for ( size_t i = 0; std::cmp_less(i, _blockSize_); i++ ) {
...
public:
	...
	static void						Init();
	void							Shutdown();
	static void						SetFixedBlocks( size_t numBlocks ) {}
	static void						SetLockMemory( bool lock ) {}
	static void						FreeEmptyBaseBlocks() {}

	type *							Alloc( const size_t num );
	type *							Resize( type *ptr, const size_t num );
	void							Free( type *ptr );
	static const char *				CheckMemory( const type *ptr );

	[[nodiscard]] static size_t		GetNumBaseBlocks() { return 0; }
	[[nodiscard]] static size_t		GetBaseBlockMemory() { return 0; }
	[[nodiscard]] size_t			GetNumUsedBlocks() const { return numUsedBlocks; }
	[[nodiscard]] size_t			GetUsedBlockMemory() const { return usedBlockMemory; }
	[[nodiscard]] static size_t		GetNumFreeBlocks() { return 0; }
	[[nodiscard]] static size_t		GetFreeBlockMemory() { return 0; }
	[[nodiscard]] static size_t		GetNumEmptyBaseBlocks() { return 0; }
...
template<class type, int baseBlockSize, int minBlockSize>
type *idDynamicAlloc<type, baseBlockSize, minBlockSize>::Alloc( const size_t num ) {
	numAllocs++;
	if ( num <= 0 ) {
		return nullptr;
	}
...
template<class type, int baseBlockSize, int minBlockSize>
type *idDynamicAlloc<type, baseBlockSize, minBlockSize>::Resize( type *ptr, const size_t num ) {
...
template<class type, int baseBlockSize, int minBlockSize>
const char *idDynamicAlloc<type, baseBlockSize, minBlockSize>::CheckMemory( const type *ptr )
{
	return nullptr;
}
...
public:
	...
	idBTreeNode<idDynamicBlock<type>,size_t> *node;			// node in the B-Tree with free blocks
};

template<class type, size_t baseBlockSize, size_t minBlockSize, memTag_t _tag_ = TAG_BLOCKALLOC>
...
public:
	...
	void							Init();
	void							Shutdown();
	void							SetFixedBlocks(size_t numBlocks );
	...
	[[nodiscard]] size_t			GetNumBaseBlocks() const { return numBaseBlocks; }
	[[nodiscard]] size_t			GetBaseBlockMemory() const { return baseBlockMemory; }
	[[nodiscard]] size_t			GetNumUsedBlocks() const { return numUsedBlocks; }
	[[nodiscard]] size_t			GetUsedBlockMemory() const { return usedBlockMemory; }
	[[nodiscard]] size_t			GetNumFreeBlocks() const { return numFreeBlocks; }
	[[nodiscard]] size_t			GetFreeBlockMemory() const { return freeBlockMemory; }
	[[nodiscard]] size_t			GetNumEmptyBaseBlocks() const;
...
template<class type, size_t baseBlockSize, size_t minBlockSize, memTag_t _tag_>
...
template<class type, size_t baseBlockSize, size_t minBlockSize, memTag_t _tag_>
void idDynamicBlockAlloc<type, baseBlockSize, minBlockSize, _tag_>::SetFixedBlocks(const size_t numBlocks ) {
	for (size_t i = numBaseBlocks; i < numBlocks; i++ ) {
		idDynamicBlock<type>* block = static_cast<idDynamicBlock<type>*>(Mem_Alloc16(baseBlockSize, _tag_));
		if ( lockMemory ) {
...
void idDynamicBlockAlloc<type, baseBlockSize, minBlockSize, _tag_>::FreeEmptyBaseBlocks() {
	idDynamicBlock<type>*next = nullptr;

	for ( idDynamicBlock<type>* block = firstBlock; block != nullptr; block = next ) {
		next = block->next;
...
template<class type, size_t baseBlockSize, size_t minBlockSize, memTag_t _tag_>
size_t idDynamicBlockAlloc<type, baseBlockSize, minBlockSize, _tag_>::GetNumEmptyBaseBlocks() const {
	size_t numEmptyBaseBlocks = 0;
	for ( idDynamicBlock<type>* block = firstBlock; block != nullptr; block = block->next ) {
...
type *idDynamicBlockAlloc<type, baseBlockSize, minBlockSize, _tag_>::Alloc( const size_t num ) {
	...
	idDynamicBlock<type>* block = AllocInternal(num);
	if ( block == nullptr) {
		return nullptr;
	}
...
type *idDynamicBlockAlloc<type, baseBlockSize, minBlockSize, _tag_>::Resize( type *ptr, const size_t num ) {
	...
	idDynamicBlock<type> *block = static_cast<idDynamicBlock<type>*>(static_cast<byte*>(ptr) - sizeof(idDynamicBlock<type>));
...
void idDynamicBlockAlloc<type, baseBlockSize, minBlockSize, _tag_>::Free( type *ptr ) {
	...
	idDynamicBlock<type> *block = reinterpret_cast<idDynamicBlock<type>*>(reinterpret_cast<byte*>(ptr) - sizeof(idDynamicBlock<type>));
...
idDynamicBlock<type> *idDynamicBlockAlloc<type, baseBlockSize, minBlockSize, _tag_>::AllocInternal( const size_t num ) {
	const size_t alignedBytes = ( num * sizeof( type ) + 15 ) & ~15;

	idDynamicBlock<type>* block = freeTree.FindSmallestLargerEqual(alignedBytes);
	if ( block != nullptr) {
		UnlinkFreeInternal( block );
	} else if ( allowAllocs ) {
		const size_t allocSize = Max( baseBlockSize, alignedBytes + sizeof(idDynamicBlock<type>) );
		block = static_cast<idDynamicBlock<type>*>(Mem_Alloc16(allocSize, _tag_));
		if ( lockMemory ) {
			//idLib::sys->LockMemory( block, baseBlockSize );
		}
#ifdef DYNAMIC_BLOCK_ALLOC_CHECK
...
idDynamicBlock<type> *idDynamicBlockAlloc<type, baseBlockSize, minBlockSize, _tag_>::ResizeInternal( idDynamicBlock<type> *block, const size_t num ) {
	...
	if ( alignedBytes > block->GetSize() ) {
		...
		// try to annex the next block if it's free
	...
	idDynamicBlock<type>* newBlock = reinterpret_cast<idDynamicBlock<type>*>(reinterpret_cast<byte*>(block) + sizeof(idDynamicBlock<type>) + alignedBytes);
#ifdef DYNAMIC_BLOCK_ALLOC_CHECK
