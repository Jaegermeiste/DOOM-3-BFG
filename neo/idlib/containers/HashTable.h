...
ID_INLINE _value_ & idHashTableT<_key_,_value_>::Set( const _key_ & key, const _value_ & value ) {
	...
	for ( ; 
		...
		nextPtr = &(node->next), node = *nextPtr ) {
			const int s = node->Compare( node->key, key );
			if ( s == 0 ) {
...
*/
template< typename _key_, class _value_ >
ID_INLINE size_t idHashTableT<_key_,_value_>::Num() const {
	return numEntries;
}
...
ID_INLINE bool idHashTable<Type>::Remove( const char *key ) {
	...
	if ( *head ) {
		for( prev = NULL, node = *head; node != nullptr; prev = node, node = node->next ) {
			if ( node->key == key ) {
				if ( prev ) {
					prev->next = node->next;
				} else {
...
*/
template< class Type >
ID_INLINE size_t idHashTable<Type>::Num() const {
	return numentries;
}
