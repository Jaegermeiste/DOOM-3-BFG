...
bool EV_DoDoor(const line_t* line, const vldoor_e type)
{
	...
	if (line)
	{
		...
		while ((secnum = P_FindSectorFromLineTag(line, secnum)) >= 0)
		{
			...
			// new door thinker
			rtn = true;
			vldoor_t* door = static_cast<vldoor_t*>(DoomLib::Z_Malloc(sizeof(*door), PU_LEVEL, nullptr));
...
void EV_VerticalDoor( line_t* line, mobj_t* thing )
{
	if (line && thing)
	{
		...
		int side = 0;	// only front ::g->sides can be used
		...
		sector_t* sec = ::g->sides[line->sidenum[side ^ 1]].sector;
		//index_t secnum = sec - ::g->sectors;
...
void P_SpawnDoorRaiseIn5Mins ( sector_t* sec )
{
	if (sec)
	{
		...
		door->topheight -= 4 * FRACUNIT;
		door->topwait = VDOORWAIT;
		door->topcountdown = 5LL * 60LL * TICRATE;
	}
}
