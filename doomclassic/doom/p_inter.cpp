 ...
 const int		num )
{
	...
	player->health += num;
	player->health = Min(player->health, MAXHEALTH);
 ...
 mobj_t*	toucher )
{
	...
	case SPR_BON1:
		...
		player->health = Min(player->health, 200);
		player->mo->health = player->health;
	...
	case SPR_BON2:
		...
		player->armorpoints = Min(player->armorpoints, 200);
		if (!player->armortype)
		{
			player->armortype = 1;
		}
